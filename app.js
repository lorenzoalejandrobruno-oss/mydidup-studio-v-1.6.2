...
class App {
    ...
    handleAuthStateChange(user) {
        ...
        const savedClass = localStorage.getItem('myDidupUserClass') || "";

        if (user || savedName) {
            if(authBox) authBox.style.setProperty('display', 'none', 'important');
            if(profileBox) profileBox.style.display = 'flex'; 
            if(logoutBtn) logoutBtn.style.display = user ? 'block' : 'none';
            if (verifySec) verifySec.style.display = (user && !user.emailVerified) ? 'block' : 'none';

            if (window.updateUserUI) window.updateUserUI(user ? user.displayName : savedName, savedClass);
            if (user) this.startSync(user);
        } else {
